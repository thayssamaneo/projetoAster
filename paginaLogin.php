...
if (isset($_POST["nomeUsuario"]) && isset($_POST["senha"])) {
    $usuario_digitado = $_POST['nomeUsuario'];
    $senha = $_POST['senha'];

    try {
        $sql = "SELECT nomeusuario, senha FROM usuarios WHERE nomeusuario = :usuario";
        ...
        if ($usuario_banco && password_verify($senha, $usuario_banco['senha'])) {
            $_SESSION['nomeUsuario'] = $usuario_banco['nomeusuario'];
...
<header class="bg-arcane-purple text-ethereal-gold shadow-lg border-b border-ethereal-gold/20 flex justify-between items-center w-full px-margin-mobile md:px-margin-desktop py-4 sticky top-0 z-50">
    <div class="flex items-center gap-4">
        ...
        <h1 class="font-manuscript-title text-4xl leading-none"><a href="index.php">Áster</a></h1>
    </div>
    <nav class="flex items-center gap-6">
        <a href="index.php" class="bg-primary-container text-secondary-container border border-secondary-container px-6 py-2 font-label-caps text-label-caps hover:bg-secondary-container hover:text-primary-container transition-all active:scale-95">
            Home
        </a>
        ...
        <div class="parchment-texture p-8 md:p-12 relative overflow-hidden border border-ink-deep/10">
            ...
            <div class="relative z-10">
                <div class="text-center mb-10">
                    <h2 class="font-manuscript-title text-5xl text-primary mb-2">Entrar</h2>
                ...
                <form class="space-y-6" action="" method="POST">
                    <div class="space-y-2">
                        <label class="block font-body-md text-ink-deep/80 text-sm italic" for="nomeUsuario">Informe seu nome de usuário</label>
                        <input class="w-full bg-surface-container-low/50 border-b-2 border-primary/20 border-t-0 border-l-0 border-r-0 py-3 px-2 font-body-md text-ink-deep placeholder:text-ink-deep/30 focus:border-ethereal-gold bg-transparent transition-all" id="nomeUsuario" placeholder="Digite seu nome" required type="text" name="nomeUsuario" value="<?php echo isset($_POST['nomeUsuario']) ? htmlspecialchars($_POST['nomeUsuario']) : ''; ?>">
                    </div>
                    
                    <div class="space-y-2">
                        <label class="block font-body-md text-ink-deep/80 text-sm italic" for="senha">Digite sua Senha</label>
                        <input class="w-full bg-surface-container-low/50 border-b-2 border-primary/20 border-t-0 border-l-0 border-r-0 py-3 px-2 font-body-md text-ink-deep placeholder:text-ink-deep/30 focus:border-ethereal-gold bg-transparent transition-all" id="senha" placeholder="••••••••" required type="password" name="senha" value="">
                    </div>
                ...
                <div class="mt-10 pt-6 border-t border-primary/10 text-center">
                    <p class="font-body-md text-on-surface-variant text-sm">
                        É novo por aqui? 
                        <a class="text-primary font-bold hover:text-secondary transition-all underline decoration-ethereal-gold/50 underline-offset-4 ml-1" href="paginaCadastro.php">
                            Cadastre-se
                        </a>
                    </p>
                </div>
